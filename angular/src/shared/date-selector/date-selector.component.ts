  ...
  implements OnInit, OnChanges
{
  ...
  @Input() type: DateSelectorEnum;
  @Input() dateTimeOptions = DATE_TIME_OPTIONS;
  @Input() label: string;
  @Input() defaultDateFilter?: DateTimeSelector
  @Output() onDateSelectorChange: EventEmitter<DateTimeSelector> =
    new EventEmitter<DateTimeSelector>();

  distanceFromAndToDate: string;

  defaultDateType: DateSelectorEnum;
  ...
  ngOnChanges(change: SimpleChanges) {
    ...
    if(change["defaultDateFilter"]){
      this.dateText = this.getDisplayText(this.defaultDateFilter.dateType, this.defaultDateFilter.fromDate, this.defaultDateFilter.toDate);
    }
  }
  ...
  changeOptions(reset?: boolean) {
    ...
    if (this.defaultDateType == DateSelectorEnum.DAY) {
      this.searchDay = moment()
        .clone()
        .startOf(unitOfTime)
  ...
  private getUnitOfTime(type: DateSelectorEnum): any {
    switch (type) {
      ...
      default: {
        return {
          unitOfTime: DateSelectorEnum.ALL.toLowerCase(),
          keyOfTime: DateSelectorEnum.ALL.toLowerCase(),
          typeDate: DateSelectorEnum.ALL,
        };
      }
    }
  }
  ...
  private getDisplayText(
    typeDate: DateSelectorEnum,
    fromDate: moment.Moment = null,
    toDate: moment.Moment = null
  ) {
    const spaceFormat = " - ";
    if (typeDate === DateSelectorEnum.ALL) {
      return "";
    }
