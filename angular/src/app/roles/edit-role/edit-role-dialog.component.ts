...
import { RoleService } from '@app/service/api/role.service';
...
export class EditRoleDialogComponent extends AppComponentBase
  ...
  public onRemoveUserFromRole(userRole){
    ...
    abp.message.confirm(
      this.l("Bạn có muốn xóa'") + userRole.userName + "'khỏi vai trò này không?",
      '',
      (result: boolean) => {
        if (result) {
          this.isLoading = true;
          this.roleService.RemoveUserFromRole(userRole.id).subscribe((rs)=>{
            if(rs){
              abp.notify.success("Xóa "+ userRole.userName + " khỏi vai trò thành công");
              this.getAllUsersInRole();
              this.isLoading = false;
            }
          },()=> this.isLoading = false)
